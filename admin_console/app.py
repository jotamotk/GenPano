...
# _fetch_query_pool_candidates moved to FastAPI in Phase 5 slice 3a.
# See backend/app/api/admin/query_pool/router.py:_fetch_candidates_paged.
...
# Phase 8 slice 8a: /api/admin/products and /api/admin/brands/<id>/products
# routes were migrated to backend/app/api/admin/products/ (FastAPI). The five
# admin_console handlers were deleted; admin.html now hits the FastAPI mount.
# Helpers above (_product_row, _coerce_product_aliases, etc.) are still
# referenced by other admin_console paths; they stay until Phase X cleanup.
...
# Phase 8 slice 8b: /api/admin/hot-topics CRUD + archive-expired + batch +
# collect routes were migrated to backend/app/api/admin/hot_topics/. The 7
# admin_console handlers were removed; admin.html now hits the FastAPI mount.
# The FastAPI collect endpoint defers to admin_console.hotspot_collectors via
# importlib so the legacy module stays on the path until Phase X cleanup
# deletes admin_console wholesale.


# /api/admin/topic-plan/{config,coverage,candidates,topics} migrated to
# FastAPI in Phase 3 B.2.a — see backend/app/api/admin/topic_plan/router.py.
# /api/admin/topic-plan/topics/{bulk-delete,<id>} migrated in Phase 3 B.3.
...
# /api/admin/topic-plan/runs/{run_id} and runs/{run_id}/stop migrated to
# FastAPI in Phase 3 B.2.a — see backend/app/api/admin/topic_plan/router.py.
...
# Flask service).
# /api/admin/prompt-matrix/{config,topics,gaps,prompts,candidates} migrated
# to FastAPI in Phase 4 slice 2 — see backend/app/api/admin/prompt_matrix/router.py.


# GET /api/admin/query-pool/candidates migrated to FastAPI in Phase 5 slice 3a.
...
# /api/admin/prompt-matrix/runs/{id} and runs/{id}/stop migrated to FastAPI
# in Phase 4 — see backend/app/api/admin/prompt_matrix/router.py.
# /api/admin/prompt-matrix/generate migrated in Phase 4 slice 3.


# /api/admin/prompt-matrix/candidates/{review,bulk-review} migrated to FastAPI
# in Phase 4 — see backend/app/api/admin/prompt_matrix/router.py.




# /api/accounts (list / import_cookies / status / reset / delete /
# auto_login) migrated to FastAPI in Phase 7 slice 7b. See
# backend/app/api/admin/accounts/router.py. Slice 7b also added admin
# auth (Depends(current_admin)) — admin_console exposed these without
# auth, which was a security gap. The /api/accounts/<id>/profiles* and
# /api/accounts/profile_counts + /api/accounts/auto_assign_profiles
# routes below are legacy product-API routes (different schema) and
# stay on Flask until Phase X.
...
# POST /api/segments/import and POST /api/segments/generate migrated to
# FastAPI in Phase 6 slice 6a-bis. See backend/app/api/admin/segments/router.py.
...
# GET / POST / import / export of /api/segments/<id>/profiles migrated
# to FastAPI in Phase 6 slice 6b. See backend/app/api/admin/segments/router.py.


# POST /api/segments/<id>/profiles/generate + GET .../generate/<job_id>
# migrated to FastAPI in Phase 6 slice 6b-bis. See
# backend/app/api/admin/segments/router.py + profile_generation.py.


# PUT / DELETE /api/segments/<id>/profiles/<pid> migrated to FastAPI in
# Phase 6 slice 6b. See backend/app/api/admin/segments/router.py.
...
# /api/admin/brand-management CRUD (industries / list / detail / create /
# update / archive) migrated to FastAPI in Phase 7 slice 7a. See
# backend/app/api/admin/brand_management/router.py.
# /api/admin/brand-management/{generate,enrich,enrich/<job_id>,import}
# migrated to FastAPI in Phase 7 slice 7a-bis. See
# backend/app/api/admin/brand_management/router.py +
# backend/app/admin/brand_management/{llm,enrich_jobs}.py.
