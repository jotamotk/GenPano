...
### 9. UFW 默认拒绝丢弃 docker bridge -> :9098 SYN，ChatGPT preflight 全部 `proxy_api_unreachable`

宿主机若启用 UFW（`Default: deny (incoming), deny (routed)`）并且未为 9098 配置 allow 规则，worker 容器（位于 `genpano_default` 网桥，172.18.0.0/16）发往 `host.docker.internal:9098` 的 SYN 会被 INPUT 链 DROP，curl 表现为 connection timed out（不是 refused）。socat 进程本身是健康的——`curl http://127.0.0.1:9098/proxies` 在宿主机上立刻返回 401。

症状：`queries.retry_reason = proxy_api_unreachable`，`latency_ms ≈ 10043`（命中 `httpx.AsyncClient(timeout=10)` 的默认超时）。只有 ChatGPT 命中此路径——`_requires_global_proxy_route` 默认仅对 chatgpt 返回 True（参见 `geo_tracker/agent/guest_executor.py:397-411`）。
