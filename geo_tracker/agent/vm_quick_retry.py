...
Boundary contract:
  ...
  - Errors map to two operator-visible codes that the FastAPI route
    ...
    These map onto QuickRetryError(code=...) so the route can switch
    on them without string parsing.
...
async def _wait_for_response_stable(
    ...
    *,
    ...
    while time.monotonic() < deadline:
        ...
        if len(text_val) != last_len:
            last_len = len(text_val)
            last_change = time.monotonic()
            last_text = text_val
        elif text_val and (time.monotonic() - last_change) >= stable_window_secs:
            return text_val
        # Use Playwright Page.wait_for_timeout (ms) so we don't need an
        # asyncio import (keeps this helper standalone-importable).
...
async def _persist_response_and_attempt(
    ...
    *,
    ...
    2. INSERT into ``query_attempts`` (production table referenced by
       ...
       silently when absent (sqlite test fixtures don't have it; the
       core attempt metadata is also reflected in the response row).
    ...
    #    ``unique=True`` on query_id; per-dialect INSERT OR REPLACE is
    #    not portable, so we branch on existence).
    try:
        existing_row = (
    ...
    # 2. Defensive table-exists check for query_attempts. Two-step to
    #    handle both sqlite (sqlite_master) and postgres
    #    (information_schema) without a hard-coded dialect string.
    exists_row = None
    try:
        exists_row = (
            await session.execute(
        ...
        # sqlite_master is missing on postgres — try information_schema.
        try:
            exists_row = (
                await session.execute(
                    sa_text(
...
def _resolve_cdp_endpoint() -> str:
    endpoint = (os.getenv("VM_QUICK_RETRY_CDP_ENDPOINT") or "").strip()
    return endpoint or DEFAULT_CDP_ENDPOINT


def _resolve_vm_id() -> str:
    vm = (os.getenv("VM_QUICK_RETRY_VM_ID") or "").strip()
    return vm or DEFAULT_VM_ID
...
async def run_quick_retry(
    *,
    ...
    """
    # Lazy-import playwright so importing this module does not pull
    # playwright into the process (the test harness can stub the
    # function via monkeypatch before any real playwright touch).
    ...
    try:
        ...
        # accepts both, matching guest_executor.py ``submit_key``).
        try:
