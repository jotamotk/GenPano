...
28. **Session 1.2 · Camoufox + MVP 3 引擎 Adapter Live 化 (进行中, Phase A 规划 2026-04-23 / 双修正预先登陆 2026-04-22)**: Session 1 交付的是 "结构框架 + 可测试纯逻辑" (决策 #22), 所有 Adapter 的 `execute()` 仍是 TIMEOUT sentinel, Camoufox / 真实 HAR / routeFromHAR 契约测试 / CAPTCHA 真集成全部延到本 Session。**双修正预先登陆 (commit 5f05229, 27 files, +971/-186, 2026-04-22)**: 主体 §1-§8 未开工前, 先把两条横切关注点固化为不可回退的基线, 避免主体实施时漂移。**Phase A 规划 (2026-04-23)**: 开工前 Frank 追问 "账号 cookies 不是应该有自动注册流程来获取的吗" + "账号的管理，是否应该属于 admin 的模块？但是这个都是共享的 pipeline 的功能？", 触发两项 Phase A 决策 — (i) 拉回鲁班 SMS live 与 auto-register live 到本 Session (规则 12 Type C scope 扩张, 登记于 C2), (ii) 固化 Platform Layer 边界 (A 段) 避免 App / Admin 双轨代码。
    ...
    **G. 双修正最终版 (2026-04-22 预先登陆)**
    ...
    - **C3 (6 枚举 response_source labeling)**: `ai_responses.response_source` 枚举 = `web_ui | api_fallback | mock_proxy | cached_replay | admin_har_replay | harness_fixture`, 所有写入必须显式 label, NO SCHEMA DEFAULT (backfill 后立即 DROP DEFAULT); 决策详情见 `docs/auto-memory/feedback_genpano_no_api_scraping.md`
...
30. **Session A0' Phase Gate 接受标准 4 轮偏离 + Bug 4 logger gap (2026-04-28)**: Session A0' (Admin 认证 Python 重写) 自 Step 8 (Smoke Checklist) 起累计 4 轮对决策 #29.C "每 Session 必须有 Vercel/Render/Fly.io 可点击 preview URL + 前后端联动 + Frank 浏览器自验" 横切要求的偏离, 全部经 Frank 显式批准, 本决策是 4 轮偏离的统一登记 + Bug 4 (logger 可观察性 gap) 转交 A1' 的承上启下。

    **A. Round 1 · Vercel preview 跳过 (Step 8, 2026-04-26)**: Frank 沙箱 (Cowork Linux) localhost:4001 对 Windows 主机不可达, 同 A0 Step 12 sandbox 限制 (决策 #24.G); Phase Gate 接受 Layer 1 (verify-session-a0prime.sh) + Layer 2 (pytest 80%+ coverage) 双绿作为 API + 行为充分证据, Vercel preview env 推到 A1' 与 frontend admin shell 联动一起落。已转交 A1', 修复路径与 frontend dashboard 联动同 PR 闭环。

    **B. Round 2 · docker compose 起本地 stack 跳过 (Step 8, 2026-04-26)**: Frank Windows 主机无可用 docker daemon (WSL2 + Docker Desktop 未配), backend FastAPI dev server 直接 uvicorn local 跑 (sqlite + asyncio loop 无外部依赖), Phase Gate 接受 uvicorn local + curl 实测作为等效证据, docker-compose.yml 编排留 A1' 与 Redis (Celery broker) + Postgres (生产 DB) 一起落。已转交 A1', 修复路径与 production stack 编排同 PR 闭环。

    **C. Round 3a · 手工 L3 9 场景重跑跳过 (Step 11, 2026-04-28)**: Step 8 Smoke Checklist 已完整跑过 G_A0.3.1-7 共 7 场景 (登录正路 / 错密 / Rate Limit 锁 / Refresh 轮换 / 登出 / 强制改密首登 / Re-Auth 30min 窗口), Step 11 重跑会重复 commit a3a3a83 / 051c9e2 / 6cc1f86 已落地的证据。Rule 12 Type C (scope creep) 适用, Phase Gate 接受 Step 8 evidence 复用 + LAYER3_REPORT §3 一对一引用作为充分证据。已转交 A1', 修复路径与 Step 8 evidence pin 同 PR 闭环。

    **D. Round 3b · 邮件实际发送跳过 + Bug 4 logger 可观察性 gap (Step 11, 2026-04-28)**: G_A0.3.8 (forgot-password) + G_A0.3.9 (reset-password) 走 mock mode 验证 — Resend SDK 未安装 + `RESEND_API_KEY` 未配, `_get_resend_client()` 返回 None → `_send()` 走 `admin_email.skipped` 分支 + 返回 `EmailResult(delivered=False)`。**4 层证据链** (LAYER3_REPORT §7): (i) curl POST /admin/api/v1/auth/forgot-password 返回 202 × 2 (incl. unknown email anti-enumeration) / (ii) `admin_password_resets` 表 1 行新增 (token_hash + user_id + expires_at + purpose='reset') 证明 endpoint 业务逻辑全跑 / (iii) uvicorn stdout 仅 access log POST 202, **缺 `admin_email.skipped` info-level 行** / (iv) Resend 未实际调用 (符合 stub 预期)。Bug 4 = (iii) 的 logger 可观察性 gap, 三候选根因: logger level 默认 WARNING 吃掉 info / 无 stdout handler attach / FastAPI uvicorn logger config 与 app logger config 未合并。**修复方向 A1'**: dictConfig + structlog + JSONRenderer 统一 logger backbone + Resend live 实际发送验证替换 mock。已在 SMOKE_CHECKLIST 转交 A1', 修复路径与 Resend live 同 PR 闭环。

    **E. Phase Gate 接受标准 (Layer 1+2 双绿 + 4 轮偏离记录在案)**: A0' 最终 Phase Gate 凭据 = `verify-session-a0prime.sh` 全绿 (12 模块 file-presence + ruff + mypy + pytest --cov-fail-under=80 + alembic + 4 admin tables + harness selftest 7/7 + bootstrap 幂等) + Layer 2 单测覆盖率 (沿用 A0 实测) + Step 8 G_A0.3.1-7 evidence 复用 + Step 11 G_A0.3.8/9 mock 验证 + 本决策 4 轮偏离登记。**A1' 转交清单**: Bug 1 / Bug 2 / Bug 3 (Step 8 known issues) + Bug 4 (本决策 D 段) + Resend live actual send 验证 + 决策 #24.C4 (AdminPasswordReset.purpose 列缺失 schema gap) + Vercel/Render preview env 接入 + docker-compose.yml 编排 + 5 个 TS 时代 .mjs scripts (check-data-contracts / ci-check / ci-harness-selftest / coverage-gap-scan / decision-log-sync-check) Python 重建或弃用 (与 Session 1' 工具链清理一并处理)。

    **F. T5 真实状态发现 + 范围调整 (2026-04-28, A1' Step 1 启动期)**: §0.5 转交清单 T5 项最初描述为 "决策 #24.C4 admin_password_resets.purpose 列缺失", 假设 TS-era Prisma schema gap 延续到 Python rewrite。A1' Step 1 启动时 T0.1 真相源验证发现 A0' baseline migration `2026_04_27_0914_55a628f2bb7d_admin_auth_baseline.py` 已交付 column (`String(length=16) NOT NULL`) + CHECK 约束 (`purpose IN ('reset', 'invitation')`), 决策 #24.C4 实际在 A0' Python rewrite 已闭合 — `backend/app/models/admin.py:21` model docstring 明确标注 `CLAUDE.md #24.C4 (purpose column gap — closed in this Session)`。**真实剩余偏差**: A0' 把 `server_default='reset'` 留在 column 上未 DROP, 偏离决策 #28.G C3 NO SCHEMA DEFAULT 纪律 (backfill 后立即 DROP DEFAULT 模式)。**T5 范围调整**: 由 "schema 补齐 (列 + CHECK + backfill)" 缩为 "仅 ALTER COLUMN DROP server_default + model 同步删 server_default 参数", 由 A1' Step 1 闭合。**为什么这是 Rule 3 偏离登记而不是 prompt bug**: §0.5 表撰写时漏读 A0' 实际交付明细, 假设 #24.C4 状态延续, 真相源验证 (T0.1 grep) 正是公约 Rule 7 "Session 完成时反查一致性" 设计来抓的 case — 抓到即按 Rule 3 登记 + 调整范围, 不退回原 prompt。轮次记录: round 7 PR (`docs/round-7-t5-scope-correction`) 修真相源层 + Step 1 commit 修代码层。

    **G. T0.3 8 张新表 schema alignment 决议 (2026-04-28, A1' Step 1 启动期)**: A1' Step 1 启动 T0.3 真相源验证发现 SESSION_A1_PRIME_PROMPT.md §5 Step 1 草稿 8 张新表全部与 PRD §4.1.4 / §4.3.7 / §4.4.8 字段定义不一致 — 4 张是命名差异 / 字段集差异, 4 张是设计哲学差异。Frank 按表分级决议:

    - **`user_moderation_actions`**: PRD 命名 (`user_id`) + spec CHECK 4 值 (`freeze`/`unfreeze`/`force_password_reset`/`soft_delete`) + 加 `expires_at` (临时冻结自动解除时间) + `reason` (合并加强)
    - **`user_activity_stats`**: 走 spec 字段集 (`query_count_30d` + `last_active_at`) 替代 PRD (`project_count` + `api_call_count_7d`), 因 30 天窗对运营页面更有信号
    - **`kg_review_queue`**: PRD 命名 (`target_type`/`target_id`/`submitted_by`/`reviewer_id`) + spec status 加 `merged` (4 值: `pending`/`approved`/`rejected`/`merged`) 支持实体合并 use case
    - **`alias_conflicts`**: 走 PRD N-候选 JSONB (`candidate_ids JSONB`) 替代 spec 2-候选 + status 状态机, 因现实别名冲突常 ≥3 候选
    - **`brand_submissions`**: PRD §4.3.7 原留白 "复用 App 侧, 不列字段", 本次首次落地 spec 完整字段集 (含 `sla_started_at` SLA 时间戳)
    - **`alerts`**: 合并 PRD 聚合设计 (`first_seen_at`/`last_seen_at`/`count` 去重) + spec 工作流设计 (`state` `open`/`acknowledged`/`resolved` + `ack_admin_id`/`ack_at` + `resolved_admin_id`/`resolved_at`), 业务上既要去重也要 ack/resolve 分离
    - **`cost_daily`**: 合并 PRD (`engine`/`category`/`amount_cny`/`token_count`) + spec (`amount_usd` 多币种, `aggregated_from`/`aggregated_to` 时间窗); 单 `id` PK + 复合 unique `(date, engine_id, industry_id, brand_id, category)`
    - **`budget_config`**: 走 spec 强 schema (`monthly_budget_usd` + `warning_threshold_pct` BETWEEN 0-100 + `hard_threshold_pct` BETWEEN 0-200) 替代 PRD key-value JSONB 通用设计, 因预算配置项有限, 强类型对前端友好

    PRD §4.1.4 / §4.3.7 / §4.4.8 同步更新为决议版本 (本 PR 内, round 8)。A1' Step 1 重启时按 round 8 后 PRD 落 alembic + SQLAlchemy 模型, 不再走 spec 草稿版本。**为什么走 Rule 3 偏离登记**: spec §5 Step 1 row 字段草稿是 Step 0 task A.R5 起草时的 best-effort, 真相源 PRD 才是权威; T0.3 真相源验证正是公约 Rule 7 设计抓的 case, 抓到 8/8 偏差不是 STOP-and-give-up, 而是 Frank 按表分级决议 + 同步双向更新 PRD (Rule 4) + 登记决策 (Rule 3) + Step 1 按决议版重启 (Rule 7 闭环)。轮次记录: round 8 PR (`docs/round-8-step1-schema-alignment`) 修真相源层 (PRD 三段 + prompt + 决策登记) + Step 1 重启修代码层。

    **H. T0.1 users 表真相源对齐 + Path B Variant 2 决议 (2026-04-28, A1' Step 3 启动期)**: A1' Step 3 启动 T0 真相源验证发现 Step 3 spec 与 DATA_MODEL §1.1 两层不一致: (1) 假设 users 表存在 (Session 4a' 未交付); (2) 假设 `users.status='suspended'/'deleted'`, 但 §1.1 schema 无 status 列 (frozen 走 user_moderation_actions 派生, soft-delete 走 deletion_requested_at 时间戳)。

    - **A. 决议**: 采纳 CC Path B Variant 2 — Step 3 实施时一并落 users 真实表 (按 DATA_MODEL §1.1 完整 schema, 10 列 + 2 索引 + 1 CHECK), 不走 upstream stub idiom (既有 4 stubs llm_responses / brands / competitors / prompts 保留)。
    - **B. spec 修正**: Y3 freeze 改写为 INSERT user_moderation_actions(action='freeze', expires_at, reason) + audit, 不再 UPDATE users; Y5 soft-delete 改写为 UPDATE users.deletion_requested_at = NOW() + INSERT user_moderation_actions(action='soft_delete') + audit, 跟 §1.1 软删除语义对齐 (30 天 grace window + cron job 在 Session 4a' 落); Y1/Y2 中 is_frozen 状态派生公式 = `(SELECT 1 FROM user_moderation_actions WHERE user_id=u.id AND action='freeze' AND (expires_at IS NULL OR expires_at > NOW()) LIMIT 1) IS NOT NULL`。
    - **C. FK materialize**: user_moderation_actions.user_id / user_activity_stats.user_id / brand_submissions.submitter_user_id 三处 String(36) plain UUID 升级为 ForeignKey('users.id') materialized FK (Step 3 follow-up alembic ALTER TABLE)。
    - **D. J5 fixture 调整**: admin 写路径白名单从 `(user.status, user.force_password_change_at)` 改为 `(users.deletion_requested_at)` (Y5 唯一允许); user.email / password_hash / name_zh / name_en 仍 NotImplementedError。force_password_change_at 在 §1.1 不存在 (走 admin_password_resets 表), J5 fixture 同步删除该字段从白名单。
    - **E. 为什么是 Rule 3 偏离登记**: Step 3 spec Type C "不动 schema" 是 Step 0 A.R5 草稿假设 users 表已存在。T0.1 grep 发现 §1.1 真相源后, schema 改动不可避免 — Round 9 PR 合法化此偏差并同步真相源 (Rule 4 双向同步) + 调整 §0.5 转交清单 T8 (Session 4a' 反正要做 users 工作, 提前切到 Step 3) + 调整 §5 Step 3 spec。
    - **F. Session 4a' 影响**: 4a' 落地时 users 表已就位, 4a' scope 缩为 + projects 表 + auth endpoints (注册 / 登录 / 邮件验证 / 密码重置自助流) + 30 天 grace window cron job; 不重做 users schema。

    **I. Step 8 · 5 个 TS 时代 .mjs scripts 评估 + Group J Harness 5 条登陆 (2026-04-29, A1' Step 8 闭合)**: §E A1' 转交清单挂账的 "5 个 TS 时代 .mjs scripts Python 重建或弃用" 项, A1' Step 8 一次性完成评估 + 处置, 不再延 Session 1' 工具链清理。**5 文件逐一决议** (read-only inspection 后 case-by-case):

    - **`scripts/check-data-contracts.mjs` · KEEP NODE**: DC1-DC7 runtime assertions on `frontend/src/data/mock.js`, 目标是 JS module (frontend prototype 仍是活跃 JSX, decision #29 frontend-as-prototype 暂不重写)。Python 没有 cheap 的 JS module evaluator, 重写需要 引入 jsdom 或硬解析 — 工程量 vs 价值不划算; 保留 Node 实现, 与 frontend 同生命周期 (frontend 走 TSX 重写时一并替换)。
    - **`scripts/ci-check.mjs` · DELETE**: 38+ frontend grep rules + Group F (TS-era backend) 的 superseded; 决策 #29 Python pivot + Step 8 `backend/scripts/ci_check.py` 已 12 条规则替换 (F1 / F4-1/2/3 / D8 / D9 / D10 + 新 J1-J5)。frontend 侧 Group A-E 规则 (CJK 泄漏 / chart-N 引擎色 / Heatmap 契约 / mentionRate 0-1 / Donut size 等) 在 jotamotk JSX 渐进式迁移到 TSX 时按需重建, 不在 A1' scope 内承诺。
    - **`scripts/ci-harness-selftest.mjs` · DELETE**: import `runAll, rules` from `ci-check.mjs`, 后者已 superseded; `backend/scripts/ci_harness_selftest.py` 已替换 (Step 8 selftest 12/12 PASS)。
    - **`scripts/coverage-gap-scan.mjs` · DELETE**: 扫描路径 `backend/src/app/api/**` 是 TS 时代 Next.js 目录结构, decision #29 Python pivot 后该目录不存在; 工具基本损坏。Python 版的覆盖率 gap 扫描不在 A1' scope (走 pytest --cov-report=html + 人工 review 流程, sufficient for MVP)。
    - **`scripts/decision-log-sync-check.mjs` · PYTHON REWRITE**: 纯 regex 算法 (CLAUDE.md `^N\. \*\*` body anchors vs DECISION_LOG.md `^| N |` 表行, 算 max + gap check), 价值高 (决策 #25 Rule 6 + Plan J D4 一行索引契约), 体量小 (~100 行)。重写为 `backend/scripts/decision_log_sync_check.py`, 行为/退出码 1:1 保留, 同 PR 验证 31 decisions in sync (实测 OK)。
    ...
    **K. ADMIN_FRONTEND_URL → ADMIN_BASE_URL 命名 Rule 1 偏离 (2026-04-29, A1' Step 10/11 间)**: A1' Step 10 (deploy infra) 实施 docker-compose.admin.yml 的 admin backend 容器 env block 时, 草稿 spec 借用 PRD §4.1 + ADMIN_PRD §4.1 频繁出现的 `ADMIN_FRONTEND_URL` 命名, 但 backend code (`backend/app/admin/auth/email.py:52` + `backend/app/core/config.py`) 实际读取的是 `ADMIN_BASE_URL`。决策 #25 Rule 1 (Source of Truth — code authoritative over spec when divergent) 强制选 code 名, 因此 docker-compose.admin.yml + render.yaml + backend/.env.example admin block 三处统一用 `ADMIN_BASE_URL`, 不引入兼容别名 / 不做 backend rename。**真相源同步**: SESSION_A1_PRIME_PROMPT.md §0.5 转交清单 T-deploy 行已加注 "ADMIN_BASE_URL = code-correct, spec 草稿误名为 ADMIN_FRONTEND_URL, Rule 1 偏离已登记 #30.K"; PRD / ADMIN_PRD 段落留待下次 mass docs sweep 一并修, 不为单字段 rename 拉独立 PR (Rule 4 软同步, 不阻 A1' Phase Gate)。**为什么是 Rule 3 偏离登记**: spec 与 code 命名漂移, 不是 bug, 是 Rule 1 自然落子; Rule 3 文档化保未来 deploy 接 frontend 时不再重蹈 (前端 fetch base URL 该用 `ADMIN_BASE_URL` env, 不要再写 `ADMIN_FRONTEND_URL`)。
...
31. **MVP scope-cut: minimax 视频解析 / minimax 网页爬取 archaeology 空集 (2026-04-28)**: A1' Step 0 重启过程中 Frank 触发的"历史 minimax 痕迹是否需复活"读检, T1-T6 read-only 扫描 (CLAUDE.md / DECISION_LOG.md / docs/ / backend/ / scripts/ / frontend/) **两条 archaeology 全部空集**: (a) **LLM-API 视频解析路径** (Doubao / DeepSeek / ChatGPT 之外引入 minimax 视频理解 API 做 video-to-text Topic discovery) 在 main 分支零代码 / 零 schema 列 / 零 Prompt 真相源段落; (b) **minimax 网页爬取 archaeology** (`engines/adapters/minimax/` 目录 + AdapterError 增 minimax 引擎枚举 + selectors.ts + api-fallback.ts) 在 main 分支同样零落地。MiniMax 作为可能的 LLM 引擎源仅在 PRD §4.2.2a "MVP 引擎枚举" 与 ADAPTER_CONTRACT §1 中以 future 候选身份出现, MVP 3 引擎 (`chatgpt | doubao | deepseek-CN`) 不含 (沿用决策 #28.G C4 口径)。
    ...
    **B. MVP scope-cut**: 本决策正式确认 MiniMax 视频/爬取双线**不进入 MVP scope**, 沿决策 #29 Python pivot 11 Session 范围继续 — Session 1' (3 引擎 Adapter 框架) / Session 1.5' (KG Platform Layer) / Session 2' / 2.1' (Planner) / Session 1.2' (Adapter live 化) 的引擎清单严格锁 `chatgpt | doubao | deepseek-CN`, 不追加 minimax。如未来 Phase 2 引入 MiniMax 引擎或视频解析 Topic discovery, 必须 (a) 新建专门 PRD 段落 + ADAPTER_CONTRACT §X.Y / (b) 新增 EngineId literal `minimax-CN` (区分 deepseek-CN 命名规范) / (c) 走完整 Adapter 契约 + Account Pool + 自动注册 / SMS 子链路 / (d) Topic 维度新增 `dimension='video'` 类型 + KG schema 扩列, 不与现有 dimension='品类'/'品牌'/'产品' 共享代码路径。

    **C. A1' Step 0 unblock**: 本 archaeology 确认是 A1' Step 0 重启的 unblock 条件之一 (其他 unblock 条件: Step 0 §0 Pre-Flight 11 grep F1-F11 / §0.5 T1-T7 transition checklist / Step 1 implementation plan draft, 见 SESSION_A1_PRIME_PROMPT.md)。Frank 不再需要为 minimax/视频复活做范围决策, A1' 后续 12 步实施按 SESSION_A1_PRIME_PROMPT.md §5 Y1-Y44 推进。

    **D. claude/* archive 处置**: 沿用决策 #29.D 既定方针 — 4 个 `claude/*` 分支 **不删除** (作为历史 sandbox 探索的 audit trail) + **不 merge** (避免污染 main 分支决策树) + **不 cherry-pick** (Python 重写按 PRD 重做架构, 不复用 TS 代码片段)。任何未来需要查询历史探索的工作可走 `git log claude/<branch>` 或 `git show claude/<branch>:<path>` 进行 read-only 考古, 不入主线。