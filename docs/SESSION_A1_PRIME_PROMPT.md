...
| **T5** | A0' baseline migration 已交付 `admin_password_resets.purpose` column + CHECK IN ('reset','invitation'), 但 `server_default='reset'` 未 DROP, 偏离决策 #28.G C3 NO SCHEMA DEFAULT 纪律 — A1' Step 1 闭合 | Schema 纪律闭合 | 决策 #24.C4 (A0' 实际已闭合) + 决策 #28.G C3 + 决策 #30.D F 段登记 | **Step 1** (alembic 8 张新表 + ALTER COLUMN DROP DEFAULT) |
...
| **T8** | A1' Step 3 落 users 真实表 (DATA_MODEL §1.1 完整 schema 10 列 + 2 索引 + 1 CHECK) + 3 处 user_id FK materialize (user_moderation_actions / user_activity_stats / brand_submissions); Session 4a' 不重做 users 只加 projects + auth endpoints + 30 天 grace cron | Schema 落地 + 跨 Session 协调 | 决策 #30.H + DATA_MODEL §1.1 | **Step 3** (alembic + FK materialize) |
| **T9** | Step 4 Module C KG scope 缩 (Option Z, 决策 #30.J): 12-15 KG 主表 endpoints (Y19-Y23 + Y28) + 7 KG 主表 alembic (kg_industries / kg_categories / kg_brands / kg_products / kg_brand_relations / kg_product_relations / kg_mined_relations) 推 Session 1.5'; A1' 内仅做 4 admin endpoints (Y24-Y27) 跑 admin 侧 3 表 (alias_conflicts / brand_submissions / kg_review_queue) | 跨 Session scope 调整 | 决策 #30.J + SESSION_1_5_PRIME_PROMPT.md §X | **Session 1.5'** |
| **T-deploy** | A1' Step 10 deploy infra 命名 Rule 1 偏离登记: docker-compose.admin.yml + render.yaml + backend/.env.example admin block 三处 env var 用 `ADMIN_BASE_URL` (code-correct, 见 backend/app/admin/auth/email.py:52) 而非 spec 草稿误名的 `ADMIN_FRONTEND_URL`; 不引兼容别名, backend 不 rename, 前端 fetch base URL 同样用 `ADMIN_BASE_URL` env; PRD / ADMIN_PRD 段落 Rule 4 软同步留下次 mass docs sweep 时一并改 | 命名纪律 (Rule 1) | 决策 #25 Rule 1 + 决策 #30.K + backend/app/admin/auth/email.py:52 | **Step 10** ✅ (本步落成) |
...
| **1** | 8 张新表 alembic + SQLAlchemy 模型 | `backend/alembic/versions/xxxx_admin_a1.py`: `user_moderation_actions` (CHECK action 4 值 + expires_at) / `user_activity_stats` (query_count_30d + last_active_at) / `kg_review_queue` (PRD 命名 + status 4 值含 merged) / `alias_conflicts` (PRD N-候选 candidate_ids JSONB) / `brand_submissions` (首次落字段 + sla_started_at) / `alerts` (PRD 聚合 first_seen/last_seen/count + spec 工作流 state/ack/resolved) / `cost_daily` (PRD engine/category/CNY/token + spec USD + aggregated_from/to + 单 id PK 复合 unique) / `budget_config` (spec 强 schema monthly_budget_usd + warning/hard threshold_pct, 取代 PRD key-value JSONB) + `models/admin/*.py` 8 模型 + CHECK 约束走 raw SQL; **8 表字段定义按 ADMIN_PRD §4.1.4 / §4.3.7 / §4.4.8 round 8 修订版为准** (见决策 #30.G); **关联转交清单 (§0.5): T5** — A0' baseline 已交付 `admin_password_resets.purpose` column + CHECK, 本步仅 `op.alter_column('admin_password_resets', 'purpose', server_default=None)` 闭合决策 #28.G C3 NO SCHEMA DEFAULT 纪律 (column 已存在, backfill 不必要); 同步删除 `backend/app/models/admin.py:124` 中 `server_default="reset"` 参数保持 model 与 schema 一致 |
...
| **3** | Module A 用户管理 5-6 endpoints + RBAC + audit + users 真实表落地 | Y1-Y6 实现 (按 ADMIN_PRD §4.1 endpoints 清单); **Step 3 含 1 张 users 表 alembic (DATA_MODEL §1.1 完整 schema 10 列 + 2 索引 + 1 CHECK) + 3 处 user_id FK materialize (user_moderation_actions / user_activity_stats / brand_submissions)** — 决策 #30.H 合法化此 schema 改动; Y3 freeze 写 `user_moderation_actions(action='freeze', expires_at)` 不写 `users.status`; Y5 soft-delete 写 `users.deletion_requested_at` + INSERT moderation_actions; is_frozen 派生公式见 ADMIN_PRD §4.1 Y2 详情字段映射段; J5 fixture 写路径白名单 = `{users.deletion_requested_at}`, 其他字段全 NotImplementedError; pytest 12 例覆盖 freeze / force-reset / soft-delete 全路径 + audit assert; **关联转交清单 (§0.5): T7** (Resend live + dictConfig 替换 mock) **+ T4** (logger gap caplog 守护) **+ T8 新加** (users 表落地, Session 4a' 不重做) |
| **4** | Module C KG 4 admin endpoints (Option Z scope, 决策 #30.J) | Y24 alias 列表 + Y25 alias resolve (N 候选 JSONB, round 9 #30.H 决议版) + Y26 brand_submissions inbox + Y27 brand_submissions approve/reject; pytest 8-10 例 J1 audit assert; alias_conflicts / brand_submissions / kg_review_queue 3 表已在 Step 1 baseline 不新增 alembic; **关联转交清单 (§0.5): T9 新加** — 12-15 KG 主表 endpoints (Y19-Y23 + Y28) + KG 7 主表 alembic 推 Session 1.5' |
