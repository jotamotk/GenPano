...
> Status: Draft → Product reference
...
> **Schema alignment (2026-04-28)**: `user_activity_stats` 字段集由 `(project_count, api_call_count_7d)` 改为 `(query_count_30d, last_active_at)`, 30 天窗对运营页面更有信号; `user_moderation_actions` CHECK 4 值 + `expires_at` 落定。
...
> **Schema alignment (2026-04-28)**: `alias_conflicts` 走 PRD N-候选 JSONB 设计 (现实别名冲突常 ≥3 候选, 强于 spec 2-候选状态机); `kg_review_queue` status 加 `merged` 第 4 值支持实体合并 use case; `brand_submissions` 首次落完整字段定义 (含 SLA 时间戳)。
...
> **Schema alignment (2026-04-28)**: `alerts` 合并 PRD 聚合设计 (`first_seen_at` / `last_seen_at` / `count` 去重) + spec 工作流设计 (`state` open/acknowledged/resolved + `ack_admin_id`/`ack_at` + `resolved_admin_id`/`resolved_at` 双 admin 分离); `cost_daily` 合并 PRD (engine/category/CNY/token) + spec (USD 多币种 + aggregated_from/to 时间窗); `budget_config` 走 spec 强 schema 取代 PRD key-value JSONB (预算配置项有限, 强类型对前端友好)。
...
与 App 的 `/api/v1/*` 完全隔离。2026-05-02 更新：不要再按 Next.js `app/admin/**` 路由组创建第二套 Admin 前端；当前只保留橙色 `/admin` Admin，后端 auth/API 代码需要先审计再决定去留。
...
> 🎯 **本段为 `admin_*` 表字段的唯一真相源**。§4.1.4 / schema / migration 均应反查本段, 不得独立重复定义。任何字段增删必须同步更新本段、schema / migration、相关测试。
...
> 2026-04-21 · Admin auth implementation added 3 fields (`force_password_change_at` / `last_password_at` / `last_login_at`). Rate limiter remains an in-memory Map + TTL implementation.
...
2026-05-02 更新：本节原先列出的 `frontend-admin/src/**` 结构锚点已废弃。不要创建或恢复 `frontend-admin/**`、`frontend/src/admin/**`、`frontend/src/pages/admin/**`、Next.js `app/admin/**` 等第二套 Admin 前端。后续 Admin UI 工作必须先确认当前橙色 `/admin` 页面实际由哪个文件渲染，再在唯一 Admin 上演进。
...
2026-05-02 更新：不要再按 Next.js `app/admin/*` 方案实现 Admin UI。当前橙色 `/admin` 是唯一 Admin UI；如需调整技术栈或迁移，必须先做独立迁移计划，不能在现有仓库中临时新建第二套 Admin 前端。
...
下一步：以本文档和 `docs/README.md` 列出的 canonical docs 为准拆分工程任务。
