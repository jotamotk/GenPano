...
user_activity_stats  (user_id, last_login_at, login_count_30d, project_count, api_call_count_7d, updated_at)
```
...
-- kg_brand_relations, kg_product_relations, discovery_logs, brand_submissions
-- Admin 侧新增:
kg_review_queue   (id, target_type, target_id, submitted_by, status, reviewer_id, reason, reviewed_at)
...
```sql
cost_daily              (date, engine, category, industry_id, brand_id, amount_cny, token_count)
budget_config           (scope, key, value_json, updated_at, updated_by)
alerts                  (id, severity, module, title, detail, first_seen_at, last_seen_at, count, status, owner_id)
