...
> 日期：2026-04-30
> 范围：以 `query_tool` 为唯一 Admin 的 GENPANO 内部运营控制台
...
- 不再建设独立的 `backend/app/admin` FastAPI Admin API。
- `query_tool/app.py` + `query_tool/templates/admin.html` 是当前 Admin 的真实入口。
- FastAPI 后端继续承担产品端 API / 用户认证 / 后续平台 API，不作为 Admin 主实现面。

这意味着后续 session 不能按“新建 Admin 前后端”的思路执行，而要围绕现有 `query_tool` Admin 做：
...
1. 当前已经跑通并验证过的业务代码，尤其是 `geo_tracker` 和 `query_tool`
...
- `backend/app/admin` 不再是活跃源码路径，当前只剩缓存文件。
- `query_tool/app.py` 是 Admin 后端主入口，包含：
  - Flask app
  - `/admin` 页面
...
- `query_tool/templates/admin.html` 是 Admin UI 主入口，已经包含：
  - 登录态 / 非登录态
  - Admin shell
...
- `query_tool/scripts/admin_reset_password.py` 已存在，用于 Admin 密码重置。
...
1. **`query_tool` Admin 是唯一 Admin**
   - Admin 页面和接口优先在 `query_tool/app.py` 与 `query_tool/templates/admin.html` 内演进。
   - 不新增 `frontend/src/admin`。
   - 不新增 `backend/app/admin` 作为 Admin 主路径。
...
4. **Admin 写操作必须有审计或明确补洞计划**
   - 当前 `query_tool` 中已有一些 mutation：retry、batch trigger、account status、reset、delete、auto login、analyzer trigger 等。
   - P0 要先盘点这些 mutation 是否审计。
   - P1 新增 mutation 必须写审计。

5. **正式 `/admin` 与临时 `/query/admin` 要分清**
   - 正式 Admin 应通过 `/admin` 暴露。
   - `query_tool` 内部 API 当前仍多为 `/api/*`。
   - 需要确认 nginx / compose 是否将 `/admin/api/*` 正确转发到 query_tool。

6. **App 产品端与 Admin 分离**
   - `frontend` 继续做用户侧 App。
   - `query_tool` 负责内部 Admin。
   - 不把 Admin 重新塞回 App React 路由。

7. **真实数据优先，少造 mock truth**
   - `query_tool` 已经连接真实查询、账号、analyzer、artifact 等数据。
   - 对尚未接入的模块，可以返回明确空状态，但不要在 UI 里制造不可追溯的假真相。

## 5. P0：大多数工作之前的基础

P0 的目标不是“搭一个新 Admin”，而是把现有 `query_tool` Admin 的边界、部署、安全、审计和契约整理清楚。
...
- 确认活跃 Admin 文件：
  - `query_tool/app.py`
  - `query_tool/templates/admin.html`
  - `query_tool/requirements.txt`
  - `query_tool/scripts/admin_reset_password.py`
  - `docker-compose.yml`
...
- 确认正式 `/admin` 是否由 query_tool 服务。
- 确认 `/admin/api/*` 是否转发到 query_tool 的 `/api/*`。
- 确认临时 `/query/admin` 是否仍可用，以及是否只作为测试入口。
- 检查 `nginx-preview.conf`、`docker-compose*.yml`、GitHub deploy workflow 是否与“query_tool 是唯一 Admin”一致。
...
- 审计 `query_tool/app.py` 中 Admin auth：
...
- 不引入第二套 Admin auth。
- 如做修补，只在 `query_tool` 体系内修补。

### P0.4 Mutation 与 Audit 盘点

任务：

- 盘点 `query_tool/app.py` 里所有 `POST` / `PUT` / `DELETE`：
  - query retry
  - batch trigger
  - mark failed
...
P1 的目标是在现有 `query_tool` Admin 上补齐 MVP 运营能力，而不是迁移到新技术栈。
...
- 基于 `query_tool` 已有 `/api/stats`、`/api/queries`、`/api/accounts`、`/api/analyzer/*` 聚合 Overview。
- 展示：
  - active alerts 或空状态
...
- 不急着实现完整 ADMIN_PRD_C_KG 九页，而是先把 query_tool 里已有 KG / analyzer 数据入口整理成可用基础。
...
重要上下文：
- 现在唯一 Admin 是 query_tool Admin。
- 不要新建 frontend/src/admin。
- 不要新建 backend/app/admin 作为 Admin 主路径。
- query_tool/app.py 和 query_tool/templates/admin.html 是 Admin 真实实现。
- geo_tracker 中已有跑通的 3 个 LLM adapter，是需要保护的资产。

目标：
整理现有 query_tool Admin 的基础边界、部署路径、认证、安全、审计和 adapter 审计准备。
...
5. 输出或修正：
   ...
   - query_tool 中所有 mutation 的 audit matrix
   - adapter contract 审计清单
6. 如果要改代码，只做最小补洞，优先修正 query_tool Admin 自身明显问题。
...
- 不要新建 frontend/src/admin。
- 不要新建 backend/app/admin 作为 Admin 主路径。
- 在 query_tool/app.py 和 query_tool/templates/admin.html 上演进。
- 不要重写 geo_tracker 的 3 个 LLM adapter。
- PRD 与已跑通代码冲突时，不默认 PRD 胜出，要列为人工决策。

目标：
在现有 query_tool Admin 上补齐 Admin MVP 运营主干，优先做真实可用的薄切片。
...
5. 对危险操作加确认和 reason。
6. 能跑就跑 query_tool / frontend / backend 的相关 smoke 或 targeted checks。
7. 最终汇报：
   - 完成了哪些 P1 切片
   - 改了哪些文件
   - 哪些 API / UI 可用
...
- query_tool/app.py 中读取 query / account / analyzer 的接口
- 相关测试和运行脚本
...
- 正式 `/admin` 必须有登录保护。
- `query_tool` Admin 不能只展示不可追溯的假数据。
