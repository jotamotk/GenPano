...
#      (视觉回归基线: auth-step0-login.png / auth-step0-register.png)
...
- **依赖**: 必须使用 Radix UI Dialog, 禁止手写 modal
...
- **依赖**: 必须使用 Radix UI Popover, 禁止手写 dropdown
...
所有字段写入 `query_executions.attempts[].rewrite_meta` JSONB 子字段 (**不建顶层列**, 避免列膨胀)。
...
- 单测 `sentiment-tiebreak.test.ts` 必须覆盖 [0.0, 0.45, 0.449, 0.45, 0.4501, 0.5, 0.549, 0.55, 0.5501, 0.6, 1.0] 11 个边界值
...
**契约**: `docs/openapi.yaml` 必须同步更新 (§4.10.4a / §4.10.4 测试策略); 契约测试应从 OpenAPI schema 生成。
...
- 与工程实现协同: PRD 中的 "本页不做" 类边界约束必须转化为空态 CTA / 导航按钮 / 空字段, 不是 copy-paste 到 messages.json
...
**数据源**: `AUTHORITY_SHARE_SERIES`, `CITATION_SOURCE_COMPOSITION`, `TOP_CITED_DOMAINS`, `TOP_CITED_PAGES`, `CITATION_TREND_BY_ENGINE`, `PR_TARGETS`, `TIER2_COVERAGE_MATRIX`, `KOL_SCORECARDS`, `CONTENT_GAP_TOPICS`, `CONTENT_GAP_PAGE_TYPE_DISTRIBUTION`, `SIMULATOR_BASELINE`, `SIMULATOR_PRESETS`。
...
4. 若未来新增引擎 (Gemini / Claude / Perplexity), 必须在本表 append 一行并同步 Planner 单测。
...
- **允许的中文硬编码**: 开发日志 / 注释 / commit message / PRD 本身
