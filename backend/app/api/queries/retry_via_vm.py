...
async def retry_query_via_vm(
    ...
    a clean response).
    """
    # 1. Load query row. The SELECT itself doubles as a table-exists
    #    probe: missing table → OperationalError → 503
    #    queries_unavailable; missing row → 404 not_found. Avoiding the
    #    ``queries_db._table_exists`` helper here is intentional —
    #    that helper queries ``information_schema.tables`` which is
    #    postgres-only and reports "missing" on sqlite even when the
    #    table is present (the conftest fixture uses sqlite).
    try:
        row = (
            (
        ...
        logger.warning("retry_via_vm: SELECT queries failed: %r", exc)
        return JSONResponse(
            status_code=503,
            content={
                "success": False,
    ...
    # 2. Dispatch to the quick-retry helper. Imported via the package
    ...
    except _vm_quick_retry.QuickRetryError as exc:
