...
def build_query_pool_llm_messages(contexts: list[dict[str, Any]]) -> list[dict[str, str]]:
    ...
    user = (
        ...
        "Prompt layer = natural user question and already owns prompt_scope: non_branded, branded, competitive; competitive prompts also own competitive_type.\n"
        "Query layer = Prompt plus Segment/Profile context. Query must preserve prompt_scope and competitive_type and must not introduce new brands or competitors.\n"
        "Inherited prompt metadata may include slot_id, product_name, scenario_axis, competitor_name, competitor_brand_id, comparison_axis, brand_context_version, and context_refs.\n"
        "Use inherited metadata only to preserve the existing Prompt meaning; do not rediscover brands, products, competitors, or context in Query Pool.\n"
        "prompt_scope rules: non_branded queries must not add brand/competitor names; branded queries keep the brand already present in Prompt; competitive queries keep the comparison/alternative/switching/shortlist framing and do not invent concrete competitors if Prompt did not name them.\n"
        "competitive metadata rule: when competitor_name is present, the Query must keep that same competitor_name and comparison_axis; never replace it with another competitor.\n"
        "context trace rule: brand_context_version is traceability metadata, not user-facing copy; never print brand_context_version or context_refs in the Query.\n"
        "competitive_type rules: direct_comparison, brand_alternative, product_alternative, switching, and shortlist must remain semantically distinct in the Query text.\n\n"
...
class QueryPoolLLMClient:
    ...
    async def _generate_query_batch(
        ...
        timeout_seconds = _clamp_int(
            os.getenv("QUERY_POOL_LLM_TIMEOUT_SECONDS"), 600, 30, 3600
        )
