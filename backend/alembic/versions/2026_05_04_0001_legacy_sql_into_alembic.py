...
preview / dev DB upgrades cleanly without touching pre-existing rows.

Tables (CREATE IF NOT EXISTS):
  - account_profile_map (002)
...
Column ALTERs (ADD COLUMN IF NOT EXISTS):
...
down_revision: Union[str, Sequence[str], None] = "20260429_user_auth"
...
def upgrade() -> None:
    ...
    """)
    # Defensive: if scheduler_config existed without engine_caps (older drop)
    op.execute(
    ...
    # ───────────────────────────────────────────────────────────────────
    # 002: query_schedules + queries.schedule_id
    # ───────────────────────────────────────────────────────────────────
    op.execute("""
    ...
    # queries.schedule_id additive (queries table is upstream Tracker domain;
    # ALTER is safe via IF NOT EXISTS)
    op.execute(
    ...
    # ───────────────────────────────────────────────────────────────────
    # 003: products + topics.product_id
    # ───────────────────────────────────────────────────────────────────
    op.execute("""
    ...
    # topics.product_id (topics is Tracker domain)
    op.execute(
    ...
    # ───────────────────────────────────────────────────────────────────
    # 004: hot_topics + prompts.hotspot_id
    # ───────────────────────────────────────────────────────────────────
    op.execute("""
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_hot_topics_industry "
        "ON hot_topics (industry);"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_hot_topics_brand "
        "ON hot_topics (brand_id);"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_hot_topics_source "
        "ON hot_topics (source, status);"
    )

    # prompts.hotspot_id (prompts is Tracker domain)
    op.execute(
