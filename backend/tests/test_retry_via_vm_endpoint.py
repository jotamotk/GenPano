...
async def existing_query(db_session: AsyncSession) -> int:
    ...
    """
    # Ensure queries table exists in this sqlite fixture. The conftest
    # creates tables from genpano_models.Base; queries is in geo_tracker.
    # Defensive create:
...
async def existing_llm_responses_table(db_session: AsyncSession) -> None:
    """Create the llm_responses table so the route's UPSERT succeeds.
...
@pytest.mark.asyncio
async def test_retry_via_vm_returns_404_when_query_missing(client, admin_operator):
    """The route loads the query first; missing rows return 404."""
    # Ensure queries table exists with NO row for this id.
    await client.post(
        "/api/queries/9999/retry_via_vm",
        json={},
    )
    # Try again with a clean assert — the first call may also hit
    # queries_unavailable if the table doesn't exist yet.
    resp = await client.post(
        "/api/queries/9999/retry_via_vm",
        json={},
    )
    # Either queries table is missing (503 queries_unavailable) or the
    # row is missing (404 not_found). Both are valid per the AC matrix.
...
async def test_retry_via_vm_happy_path_with_real_captured_rawtext(
    ...
    # AC-4 (cross-check): the stub was called with the right query_id
